...
# Setup OpenSIPS/Sipcapture from Packages
echo
echo "**************************************************************"
...
case $DIST in
    ...
    apt-get update -qq && apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 049AD65B
    echo "deb http://apt.opensips.org jessie 2.2-releases" >>/etc/apt/sources.list
    apt-get update -qq && apt-get install -f -yqq rsyslog opensips opensips-geoip-module opensips-json-module opensips-mysql-module opensips-regex-module opensips-restclient-module geoip-database geoip-database-extra
    ...
    ln -s /usr/lib64 /usr/lib/x86_64-linux-gnu/
		...
		sed -i -e "s/#USER/USER/g" /etc/default/opensips
		sed -i -e "s/#GROUP/GROUP/g" /etc/default/opensips

		# Test the syntax.
		# opensips -c $PATH_OPENSIPS_CFG
