    ...
    'DEBIAN')
	   ...
	   MYSQL="mysql"
		...
		function MYSQL_INITIAL_DATA_LOAD () {
		  ...
		  mysql -u "$sqlhomeruser" -p"$sqlhomerpassword" < $SQL_LOCATION/homer_databases.sql
		  mysql -u "$sqlhomeruser" -p"$sqlhomerpassword" < $SQL_LOCATION/homer_user.sql
		  
		  echo "Creating Tables..."
		  mysql -u "$sqlhomeruser" -p"$sqlhomerpassword" homer_data < $SQL_LOCATION/schema_data.sql
		  mysql -u "$sqlhomeruser" -p"$sqlhomerpassword" homer_configuration < $SQL_LOCATION/schema_configuration.sql
		  mysql -u "$sqlhomeruser" -p"$sqlhomerpassword" homer_statistic < $SQL_LOCATION/schema_statistic.sql
		  
		  # echo "Creating local DB Node..."
		  mysql -u "$sqlhomeruser" -p"$sqlhomerpassword" homer_configuration -e "INSERT INTO node VALUES(1,'mysql','homer_data','3306','"$DB_USER"','"$DB_PASS"','sip_capture','node1', 1);"
